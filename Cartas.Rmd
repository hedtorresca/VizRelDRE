...
selectInput('year', 'Seleccione un año', c("Todos", "2016", "2017"  ,"2018",  "2019"),
                selected="Todos")
...
selectedYear <- reactive({
    ...
    if (input$year %in% c("2017"  ,"2018",  "2019")) {
...
selectedIssue <- reactive({
    ...
    if (input$issue %in% levels(as.factor(cartas$tramite))) {
      datos <- selectedYear()[selectedYear()$tramite == input$issue,]
 return(datos)
    } else {
            datos <- selectedYear()
          return(datos)
...
selectedCountry <- reactive({
    ...
    if (input$pais %in% levels(as.factor(cartas$País))) {
      datos <- selectedIssue()[selectedIssue()$País == input$pais,]
 return(datos)
    } else {
            datos <- selectedIssue()
          return(datos)
...
cant_cartas <- reactive({
  cant_cartas <-selectedYear() %>% group_by(País) %>% summarise(Total=n())
  cant_cartas <- na.omit(cant_cartas)
return(cant_cartas)})

cant_cartasPIP <-  reactive({
  cant_cartasPIP <-selectedYear()[cartas$tramite== "PIP",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasPIP <- na.omit(cant_cartasPIP)
return(cant_cartasPIP)})


cant_cartasAPC <- reactive({ 
cant_cartasAPC <- selectedYear()[selectedYear()$tramite== "APC",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasAPC <- na.omit(cant_cartasAPC)
return(cant_cartasAPC)})


cant_cartasPIP5 <-  reactive({
  cant_cartasPIP5 <- selectedYear()[selectedYear()$tramite== "PIP5",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasPIP5 <- na.omit(cant_cartasPIP5)
return(cant_cartasPIP5)})


cant_cartasTP12 <- reactive({ cant_cartasTP12 <-selectedYear()[selectedYear()$tramite== "TP12",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasTP12 <- na.omit(cant_cartasTP12)
return(cant_cartasTP12)})

cant_cartasTP7 <-  reactive({cant_cartasTP7 <- selectedYear()[selectedYear()$tramite== "TP7",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasTP7 <- na.omit(cant_cartasTP7)
return(cant_cartasTP7)})


cant_cartasAUIP <-  reactive({cant_cartasAUIP <-selectedYear()[selectedYear()$tramite== "PIP6",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasAUIP <- na.omit(cant_cartasAUIP)
return(cant_cartasAUIP)})

cant_cartasTP4 <-  reactive({cant_cartasTP4 <- selectedYear()[selectedYear()$tramite== "TP4",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasTP4 <- na.omit(cant_cartasTP4)
return(cant_cartasTP4)})


cant_cartasVISA <-  reactive({cant_cartasVISA <- selectedYear()[selectedYear()$tramite== "VISA",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasVISA <- na.omit(cant_cartasVISA)
return(cant_cartasVISA)})


cant_cartasPIP7 <-  reactive({cant_cartasPIP7 <- selectedYear()[selectedYear()$tramite== "PIP7",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasPIP7 <- na.omit(cant_cartasPIP7)
return(cant_cartasPIP7)})


cant_cartasTP5 <-  reactive({cant_cartasTP5 <- selectedYear()[selectedYear()$tramite== "TP5",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasTP5 <- na.omit(cant_cartasTP5)
return(cant_cartasTP5)})


cant_cartasPIP2 <-  reactive({cant_cartasPIP2 <- selectedYear()[selectedYear()$tramite== "PIP2",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasPIP2 <- na.omit(cant_cartasPIP2)
return(cant_cartasPIP2)})


cant_cartasTP1 <-  reactive({cant_cartasTP1 <- selectedYear()[selectedYear()$tramite== "TP1",] %>% group_by(País) %>% summarise(Total=n())
cant_cartasTP1 <- na.omit(cant_cartasTP1)
return(cant_cartasTP1)})


cant_cartasTP6 <-  reactive({cant_cartasTP6 <- selectedYear()[selectedYear()$tramite== "TP6",] %>% group_by(País) %>% summarise(Total=n())
